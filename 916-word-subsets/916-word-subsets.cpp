   ...
   vector<string> wordSubsets(vector<string>& words1, vector<string>& words2) {
    ...
    for(int i=0;i<words2.size();i++){
        ...
        for(auto ch:current){       
            ...
            freq2[ch-'a']=max(freq2[ch-'a'], temp[ch-'a']);   //update freq2 only if temp[ch-'a'] is greater than freq2[ch-'a']
        }
    }
    ...
    for(int i=0;i<words1.size();i++){
        ...
        for(auto ch: current){      //update freq1
            freq1[ch-'a']++;
        }
        
        for(int i=0;i<26;i++){           
            if(freq2[i]>freq1[i]){       //if for any alphabet, it's count (freq2) is more than the count in freq1, 
